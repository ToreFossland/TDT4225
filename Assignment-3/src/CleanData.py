...
class CleanData:
    def __init__(self):
        self.cwd = "/Users/torefossland/VSCode/TDT4225/Assignment-3/"
        self.labels = self.get_labels()
        self.users = []
        self.activities = []
        self.trackpoints = []
        self.date_format = "%Y-%m-%d %H:%M:%S"
        self.userColumns = ['_id', 'has_labels']
        self.activityColumns = ['_id', 'user_id','transportation_mode', 'start_date_time', 'end_date_time']
    ...
    def make_df_activity_trackPoints(self):
        ...
        for i, row in self.users.iterrows():
            id, has_labels = row['_id'], row['has_labels']
            folder = path + id
            ...
            for j, file in enumerate(glob.glob(folder +'/Trajectory/*.plt')):
                ...
                for start_date_time, end_date_time, transportation_mode in labels:
                    if start_date_time == start_date and end_date_time == end_date:
                        activity_df = pd.DataFrame([[activity_counter, row['_id'], transportation_mode,  start_date, end_date]], columns=self.activityColumns)
                        self.activities.append(activity_df)
                        activity_counter += 1
                        inserted = True
                        break
                if not inserted:
                    activity_df = pd.DataFrame([[activity_counter, row['_id'], None,  start_date, end_date]], columns=self.activityColumns)
                    activity_counter += 1
                    self.activities.append(activity_df)
